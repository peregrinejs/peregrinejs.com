...
export const meta = { title: 'Comparison with Cordova and Capacitor' }
...
[React Native](https://reactnative.dev), [Flutter](https://flutter.dev), and
[Turbo Native](https://turbo.hotwired.dev) are out of scope for this page.
...
Frame](architecture#web-frames) that the web layer will be able to access. This
is what's known as the [Remote Interface](architecture#remote-interfaces).
...
- Peregrine is primarily managed and maintained by
  [@imhoffd](https://twitter.com/imhoffd), an Ionic alumni.

## Purpose

When comparing products, it's important to consider their purpose and the
intentions of the stewards who create and maintain them. Both Cordova and
Capacitor have corporate backgrounds, so we can at least assume there was a
monetary purpose for each--however, both are free to use for individuals and
teams alike. Peregrine, on the other hand, is _not_ free to use for closed
source projects.

In other words, the purpose of Peregrine is to sell Peregrine itself: to
provide the best product and be a strong candidate for your subsequent
projects. I'll leave the purpose of Cordova and Capacitor up to you to
discover.
