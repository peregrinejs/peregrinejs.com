...
3. Sync your project with Gradle files by clicking 'Sync Now' and then click
   'Make Project' to download and install the dependency.
...
<Callout type="info" title="Combine framework">

We'll be using the [Combine
framework](https://developer.apple.com/documentation/combine) to asynchronously
send native events to the web layer. This official library powers reactive
state in SwiftUI views and is a modern and declarative way to process values
over time.
...
<Callout type="info" title="Kotlin coroutines">

We'll be using [Kotlin
coroutines](https://developer.android.com/kotlin/coroutines) to asynchronously
send native events to the web layer.
[Coroutines](https://kotlinlang.org/docs/coroutines-guide.html) and
[flows](https://kotlinlang.org/docs/flow.html) are quickly becoming the
standard way of writing asynchronous code in Android. Coroutines have
first-class support in AndroidX and the Kotlin language itself.
